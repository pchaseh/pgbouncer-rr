## Disclaimer
This repository is a fork of [pgbouncer-fast-switchover](https://github.com/awslabs/pgbouncer-fast-switchover), specifically up until [8056484](https://github.com/awslabs/pgbouncer-fast-switchover/commit/8056484c816899fc97f2f3a3f2dff1ff11393ddf). The reasons for this are as follows:
- I don't require pgbouncer to be capable of failing over -  that's a feature I'd rather leave to other components in a High-Availability Postgres setup while letting this patch serve its original intent (routing and rewriting of queries)
- This commit, when applied on top of a newer pgbouncer version, will introduce a crash due to the fact that it tries combining database parsing and pool initialization in one step for its topology queries (see [PR #102](https://github.com/awslabs/pgbouncer-fast-switchover/pull/102))

As such, I claim no credit with regards to pgbouncer-rr. All rights are held by Amazon, and my sole intent is to bring the routing/rewriting functionality provided by the original authors to newer pgbouncer versions.

## Support
This patch currently targets pgbouncer 1.23
...
The [pgbouncer-rr](https://github.com/pchaseh/pgbouncer-rr-patch) project is based on [pgbouncer](https://pgbouncer.github.io/usage.html#description), an open source PostgreSQL connection pooler. It adds two new significant features:
